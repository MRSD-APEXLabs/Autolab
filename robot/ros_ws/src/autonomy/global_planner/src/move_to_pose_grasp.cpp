    ...
    rclcpp::Node::SharedPtr                                         node)
{
    ...
    // Poll until joints reach target or timeout
    auto start = node->now();
    bool execution_complete = false;

    while ((node->now() - start).seconds() < 15.0)  // max 15s timeout
    {
        auto current_state = arm_group.getCurrentState(1.0);
        std::vector<double> current_joints;
        current_state->copyJointGroupPositions(
            current_state->getJointModelGroup(arm_group.getName()), 
            current_joints);

        // Check if current joints match goal joints (last point of plan)
        const auto &goal_joints = plan.trajectory_.joint_trajectory.points.back().positions;
        
        double max_error = 0.0;
        for (size_t i = 0; i < goal_joints.size(); ++i)
            max_error = std::max(max_error, std::abs(current_joints[i] - goal_joints[i]));

        if (max_error < 0.01)  // within 1 degree
        {
            RCLCPP_INFO(node->get_logger(), "Arm reached target (max_error=%.4f). Proceeding to gripper.", max_error);
            execution_complete = true;
            break;
        }

        RCLCPP_INFO(node->get_logger(), "Waiting for arm... max_error=%.4f", max_error);
        std::this_thread::sleep_for(std::chrono::milliseconds(200));
    }

    if (!execution_complete)
        RCLCPP_WARN(node->get_logger(), "Arm execution timed out — proceeding anyway.");


    
    // FORCE an update of the internal robot state
    arm_group.setStartStateToCurrentState();
