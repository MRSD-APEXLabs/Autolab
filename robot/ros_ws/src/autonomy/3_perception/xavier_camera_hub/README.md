...
This folder is plain Python 3.8 (stdlib `http.server`, numpy, OpenCV), not a ROS package (see `COLCON_IGNORE`).
...
Deploy from the Thor with `./deploy.sh`. It rsyncs this folder to the Xavier and keeps these runtime files
there:
- `state.json`
- logs
- cached `SN*.conf`
...
- On the Thor, from `2_manipulation/`: `../.venv/bin/python -m pytest -q -p no:cacheprovider xavier_camera_hub/tests`
