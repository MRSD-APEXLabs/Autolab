...
# autolab - A convenience tool for Autolab development
# 
# This script provides a unified interface for common development tasks
# in the Autolab project, including setup, installation, and container management.
...
MODULES_DIR="$PROJECT_ROOT/.autolab/modules"
...
function print_usage {
    echo -e "${BOLDCYAN}AutoLab Development Tool ($SCRIPT_DIR)${NC}"
    echo ""
    echo "Usage: autolab <command> [options]"
    echo ""
    ...
    echo ""
    echo "For more information on a command, run: autolab help <command>"
    echo "To see all available commands, run: autolab commands"
    echo ""
    echo "Note: The autolab command will automatically use the autolab.sh script from the"
    echo "current directory or nearest parent directory containing an AutoLab repository."
}
...
function print_command_help {
    ...
    # Print command-specific help
    echo -e "${BLUE}autolab $command${NC} - $help_text"
    echo ""
    
    # Command-specific usage and options
    case "$command" in
        install)
            echo "Usage: autolab install [options]"
            echo ""
            echo "Options:"
            ...
            ;;
        setup)
            echo "Usage: autolab setup [options]"
            echo ""
            echo "Options:"
            echo "  --no-shell    Don't modify shell configuration"
            echo "  --no-config   Skip configuration tasks (Isaac Sim, Nucleus, Git hooks)"
            echo ""
            echo "This command adds an 'autolab' function to your shell profile that will"
            echo "automatically find and use the autolab.sh script from the current directory"
            echo "or nearest parent directory containing an AutoLab repository."
            ;;
        up)
            echo "Usage: autolab up [service_name] [options]"
            echo ""
            echo "Options:"
        ...
        logs)
            ...
            ;;
        test)
            echo "Usage: autolab test [options]"
            echo ""
            echo "Options:"
...
function check_docker {
    if ! command -v docker &> /dev/null; then
        log_error "Docker is not installed. Run 'autolab install' first."
        exit 1
    fi
...
function find_container {
    ...
    if [ "$match_count" -eq 1 ]; then
        ...
        if [ "$selection" = "q" ]; then
            ...
            log_info "Tip: Next time, you can directly use 'autolab connect $container_name' for this container"
            return 0
        else
        ...
        if command -v docker &> /dev/null && [ "$force" = false ]; then
            ...
            case "$OS" in
                ...
                darwin)
                    ...
                    log_info "After installation, run 'autolab setup' to complete the configuration"
                    ;;
...
function cmd_setup {
    log_info "Setting up AutoLab environment..."
    ...
    if [ "$modify_shell" = true ]; then
        ...
        if grep -q "# AutoLab function" "$shell_profile"; then
            log_info "'autolab' function already registered"
        else
            log_info "Adding 'autolab' function in $shell_profile"
            echo "" >> "$shell_profile"
            echo "# AutoLab function" >> "$shell_profile"
            echo 'function autolab() {' >> "$shell_profile"
            echo '    # Start from the current directory and look for autolab.sh' >> "$shell_profile"
            echo '    local current_dir="$(pwd)"' >> "$shell_profile"
            echo '    local found_scripts=()' >> "$shell_profile"
            echo '    local script_path=""' >> "$shell_profile"
            echo '' >> "$shell_profile"
            echo '    # First check if there is an autolab.sh in the current directory or any parent directory' >> "$shell_profile"
            echo '    while [[ "$current_dir" != "" ]]; do' >> "$shell_profile"
            echo '        if [[ -f "$current_dir/autolab.sh" ]]; then' >> "$shell_profile"
            echo '            found_scripts+=("$current_dir/autolab.sh")' >> "$shell_profile"
            ...
            echo '        echo -e "\033[0;31m[ERROR]\033[0m No autolab.sh script found in the current directory or any parent directory."' >> "$shell_profile"
            echo '        return 1' >> "$shell_profile"
            echo '    elif [[ ${#found_scripts[@]} -gt 1 ]]; then' >> "$shell_profile"
            echo '        echo -e "\033[0;31m[ERROR]\033[0m Multiple autolab.sh scripts found in the directory hierarchy:"' >> "$shell_profile"
            echo '        for script in "${found_scripts[@]}"; do' >> "$shell_profile"
            echo '            echo "  - $script"' >> "$shell_profile"
            echo '        done' >> "$shell_profile"
            echo '        echo "Please cd to the specific AutoLab repository you want to use."' >> "$shell_profile"
            echo '        return 1' >> "$shell_profile"
            ...
            echo '    fi' >> "$shell_profile"
            echo '}' >> "$shell_profile"
            echo "Added to $shell_profile. Please restart your shell or run 'source $shell_profile'. Then you'll be able to use the 'autolab' command from any sub-directory."
        fi
    fi
...
function cmd_status {
    ...
    log_info "AutoLab container status:"
...
function register_builtin_commands {
    ...
    COMMAND_HELP["setup"]="Configure AutoLab settings and add to shell profile"
...
function list_commands {
    echo -e "${BLUE}All Available AutoLab Commands:${NC}"
    echo ""
