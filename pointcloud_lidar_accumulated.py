...
PC_COUNT = 15
USE_PC_CACHE = True   # False = never load/save point cloud caches, always re-buffer
...
class WSVisualizer(Node):
    def __init__(self):
        ...
        self._lidar_pc_buffer: List[np.ndarray] = []
    ...
    def lidar_callback(self, msg: PointCloud2):
        ...
        if self._lidar_pcs is not None:
        ...
        self._lidar_pc_buffer.append(pc)
        self.get_logger().info(
            f"Buffering LiDAR point clouds: {len(self._lidar_pc_buffer)}/{PC_COUNT}"
        )

        if len(self._lidar_pc_buffer) >= PC_COUNT:
            self._lidar_pcs = np.vstack(self._lidar_pc_buffer)
            self._lidar_pc_buffer.clear()
            if USE_PC_CACHE:
                np.save(LIDAR_PC_CACHE_PATH, self._lidar_pcs)
                self.get_logger().info(
                    f"LiDAR point cloud accumulated: {self._lidar_pcs.shape[0]} points. "
                    f"Saved to {LIDAR_PC_CACHE_PATH}"
                )
            else:
                self.get_logger().info(
                    f"LiDAR point cloud accumulated: {self._lidar_pcs.shape[0]} points (cache disabled)"
                )
            self.lidar_pc_pub.publish(
                self.create_pc2(self._lidar_pcs, self._lidar_frame_id)
            )
        ...
        if pc is not None and self._pcs is None:
            ...
            if len(self._pc_buffer) >= PC_COUNT:
                ...
                if USE_PC_CACHE:
                    ...
                    self.get_logger().info(
                        f"Point cloud accumulated: {self._pcs.shape[0]} points (cache disabled)"
                    )
    ...
    async def run(self):
        spin_task = asyncio.create_task(self._spin_ros())
        try:
            while rclpy.ok():
                try:
